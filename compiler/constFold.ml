...
let findi x env =
  (match M.find x env with Int(i, _) -> i | _ -> raise Not_found)
let findf x env =
  (match M.find x env with Float(d, _) -> d | _ -> raise Not_found)
let findt x env =
  (match M.find x env with Tuple(ys, _) -> ys | _ -> raise Not_found)

let rec g env = function
  | Var(x, p) when memi x env -> Int(findi x env, p)
  | Neg(x, p) when memi x env -> Int(-(findi x env), p)
  | Add(x, y, p) when memi x env && memi y env ->
      Int(findi x env + findi y env, p)
  | Sub(x, y, p) when memi x env && memi y env ->
      Int(findi x env - findi y env, p)
  | Mul(x, y, p) when memi x env && memi y env ->
      Int(findi x env * findi y env, p)
  ...
  | FSub(x, y, p) when memf x env && memf y env ->
      Float(findf x env -. findf y env, p)
  | FMul(x, y, p) when memf x env && memf y env ->
      Float(findf x env *. findf y env, p)
  ...
  | IfLE(x, y, e1, e2, _) when memi x env && memi y env ->
      if findi x env <= findi y env then g env e1 else g env e2
  | IfLE(x, y, e1, e2, _) when memf x env && memf y env ->
      if findf x env <= findf y env then g env e1 else g env e2
  ...
  | LetTuple(xts, y, e, p) when memt y env ->
      (* 型検査を通っているので数は合うこと前提、タプルをバラバラに *)
      List.fold_left2
        (fun e' xt z -> Let(xt, Var(z, p), e', p)) (g env e) xts (findt y env)
